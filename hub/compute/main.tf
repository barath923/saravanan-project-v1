...
locals {
  nsg_map = {
    ...
    shared = azurerm_network_security_group.shared_nsg.id
  }
}
...
resource "azurerm_network_security_group" "jump_vm_nsg" {
  ...
  security_rule {
    ...
    source_address_prefix      = "Internet" # Allow SSH from any source IP (be careful with this in production)
    destination_address_prefix = "*"
  }

  security_rule {
  name                       = "AllowASROutbound"
  priority                   = 409
  direction                  = "Outbound"
  access                     = "Allow"
  protocol                   = "Tcp"
...
resource "azurerm_network_interface" "nic" {
  ...
  ip_configuration {
    ...
    subnet_id = each.value.subnet_type == "mgmt" ? var.subnet_mgmt_id : var.subnet_shared_id
...
resource "azurerm_managed_disk" "windows_data_disk_10gb" {
  for_each = {
    for k, v in local.all_vms_map : k => v if v.os_type == "windows"
  }
  depends_on           = [azurerm_windows_virtual_machine.windows]
  name                 = "${each.key}-data-10gb"
  location             = var.location
  resource_group_name  = var.resource_group_name
  storage_account_type = "Standard_LRS"
  create_option        = "Empty"
...
resource "azurerm_virtual_machine_data_disk_attachment" "windows_data_disk_attach" {
  ...
  managed_disk_id    = azurerm_managed_disk.windows_data_disk_10gb[each.key].id
...
resource "azurerm_linux_virtual_machine" "linux" {
  ...
  admin_ssh_key {
    ...
    public_key = file("${path.module}/mykeyone.pub")
  }
