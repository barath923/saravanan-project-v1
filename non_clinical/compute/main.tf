...
locals {
  nsg_map = {
    mgmt   = azurerm_network_security_group.non_clinical_jump_vm_nsg.id
    subnet1 = azurerm_network_security_group.non_clinical_shared_nsg.id
    subnet2 = azurerm_network_security_group.non_clinical_shared_nsg.id
  }
}

# NSG for jump vm
resource "azurerm_network_security_group" "non_clinical_jump_vm_nsg" {
  name                = "non-clinical-jump-nsg"
...
#shared nsg
resource "azurerm_network_security_group" "non_clinical_shared_nsg" {
  name                = "non-clinical-shared-nsg"
  location            = var.non_clinical_location
  resource_group_name = var.non_clinical_resource_group_name
...
resource "azurerm_network_interface" "nic" {
  ...
  ip_configuration {
    ...
    subnet_id = each.value.subnet_type == "mgmt" ? var.management_subnet_id : each.value.subnet_type == "subnet1" ? var.subnet_one_id : var.subnet_two_id
